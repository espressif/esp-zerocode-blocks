{
    /* No standard EVSE device type exists — use esp.device.other. */
    ...
    s_{{prefix_lc}}_enabled_param = esp_rmaker_param_create(
        "Charging Enabled", ESP_RMAKER_PARAM_TOGGLE,
        esp_rmaker_bool(init.b), PROP_FLAG_READ | PROP_FLAG_WRITE);
    if (s_{{prefix_lc}}_enabled_param) {
        esp_rmaker_param_add_ui_type(s_{{prefix_lc}}_enabled_param, ESP_RMAKER_UI_TOGGLE);
    }
    esp_rmaker_device_add_param(s_{{prefix_lc}}_device, s_{{prefix_lc}}_enabled_param);
    esp_rmaker_device_assign_primary_param(s_{{prefix_lc}}_device, s_{{prefix_lc}}_enabled_param);
    esp_rmaker_node_add_device(node, s_{{prefix_lc}}_device);
    ESP_LOGI(TAG, "{{prefix_lc}}: RainMaker EVSE device '{{cfg.label}}'");
}
