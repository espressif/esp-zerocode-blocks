{
    ...
    s_{{prefix_lc}}_leak_param = esp_rmaker_param_create(
        "Leak", ESP_RMAKER_PARAM_TOGGLE, esp_rmaker_bool(init.b), PROP_FLAG_READ);
    esp_rmaker_device_add_param(s_{{prefix_lc}}_device, s_{{prefix_lc}}_leak_param);
    esp_rmaker_device_assign_primary_param(s_{{prefix_lc}}_device, s_{{prefix_lc}}_leak_param);
    esp_rmaker_node_add_device(node, s_{{prefix_lc}}_device);
    ESP_LOGI(TAG, "{{prefix_lc}}: RainMaker Water Leak Detector device '{{cfg.label}}'");
}
