{
    s_{{prefix_lc}}_device = esp_rmaker_device_create(
        "{{cfg.label}}", ESP_RMAKER_DEVICE_AIR_CONDITIONER, NULL);
    ...
    if (s_{{prefix_lc}}_setpoint_param) {
        ...
        esp_rmaker_param_add_bounds(s_{{prefix_lc}}_setpoint_param,
            esp_rmaker_float(16.0f), esp_rmaker_float(32.0f), esp_rmaker_float(0.5f));
    }
    ...
    ESP_LOGI(TAG, "{{prefix_lc}}: RainMaker Air Conditioner device '{{cfg.label}}'");
}
