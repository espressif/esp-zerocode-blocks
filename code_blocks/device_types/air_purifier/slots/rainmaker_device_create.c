{
    s_{{prefix_lc}}_device = esp_rmaker_device_create(
        "{{cfg.label}}", ESP_RMAKER_DEVICE_FAN, NULL);
    ...
    s_{{prefix_lc}}_speed_param = esp_rmaker_param_create(
        ESP_RMAKER_DEF_SPEED_NAME, ESP_RMAKER_PARAM_SPEED,
        esp_rmaker_int(init.u8), PROP_FLAG_READ | PROP_FLAG_WRITE);
    if (s_{{prefix_lc}}_speed_param) {
        esp_rmaker_param_add_ui_type(s_{{prefix_lc}}_speed_param, ESP_RMAKER_UI_SLIDER);
        esp_rmaker_param_add_bounds(s_{{prefix_lc}}_speed_param,
            esp_rmaker_int(0), esp_rmaker_int(100), esp_rmaker_int(1));
    }
    esp_rmaker_device_add_param(s_{{prefix_lc}}_device, s_{{prefix_lc}}_speed_param);
    esp_rmaker_device_assign_primary_param(s_{{prefix_lc}}_device, s_{{prefix_lc}}_speed_param);
    esp_rmaker_node_add_device(node, s_{{prefix_lc}}_device);
    ESP_LOGI(TAG, "{{prefix_lc}}: RainMaker Air Purifier device '{{cfg.label}}'");
}
