{
    ezb_zha_temperature_sensor_config_t {{prefix_lc}}_cfg =
        EZB_ZHA_TEMPERATURE_SENSOR_CONFIG();
    ezb_af_ep_desc_t ep_desc =
        ezb_zha_create_temperature_sensor({{cfg.zb_endpoint}}, &{{prefix_lc}}_cfg);
    if (ep_desc == EZB_INVALID_AF_EP_DESC) {
        ESP_LOGE(TAG, "{{prefix_lc}}: failed to create Zigbee Temperature Sensor endpoint");
    ...
    ESP_LOGI(TAG, "{{prefix_lc}}: Zigbee Temperature Sensor endpoint %d", {{cfg.zb_endpoint}});
}
