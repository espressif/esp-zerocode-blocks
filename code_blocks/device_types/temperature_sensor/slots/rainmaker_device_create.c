{
    s_{{prefix_lc}}_device = esp_rmaker_device_create(
        "{{cfg.label}}", ESP_RMAKER_DEVICE_TEMP_SENSOR, NULL);
    ...
    /* Standard read-only Temperature param (esp.param.temperature, float °C). */
    s_{{prefix_lc}}_temperature_param = esp_rmaker_temperature_param_create(
        ESP_RMAKER_DEF_TEMPERATURE_NAME, init.i16 / 100.0f);
    esp_rmaker_device_add_param(s_{{prefix_lc}}_device, s_{{prefix_lc}}_temperature_param);
    esp_rmaker_device_assign_primary_param(s_{{prefix_lc}}_device, s_{{prefix_lc}}_temperature_param);
    esp_rmaker_node_add_device(node, s_{{prefix_lc}}_device);
    ESP_LOGI(TAG, "{{prefix_lc}}: RainMaker Temperature Sensor device '{{cfg.label}}'");
}
