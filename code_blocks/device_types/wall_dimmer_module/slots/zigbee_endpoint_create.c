{
    ...
    if (ep_desc == EZB_INVALID_AF_EP_DESC) {
        ESP_LOGE(TAG, "{{prefix_lc}}: failed to create Zigbee Dimmable Light endpoint");
    ...
    ESP_LOGI(TAG, "{{prefix_lc}}: Zigbee Dimmable Light (wall dimmer) endpoint %d", {{cfg.zb_endpoint}});
}
