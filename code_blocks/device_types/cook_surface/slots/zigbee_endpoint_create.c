{
    ...
    if (ep_desc == EZB_INVALID_AF_EP_DESC) {
        ESP_LOGE(TAG, "{{prefix_lc}}: failed to create Zigbee Thermostat (cook surface) endpoint");
    ...
    ESP_LOGI(TAG, "{{prefix_lc}}: Zigbee Thermostat (cook surface) endpoint %d", {{cfg.zb_endpoint}});
}
