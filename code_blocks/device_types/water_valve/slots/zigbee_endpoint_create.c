{
    ...
    if (ep_desc == EZB_INVALID_AF_EP_DESC) {
        ESP_LOGE(TAG, "{{prefix_lc}}: failed to create Zigbee Water Valve (Mains Power Outlet) endpoint");
    ...
    ESP_LOGI(TAG, "{{prefix_lc}}: Zigbee Water Valve (Mains Power Outlet) endpoint %d", {{cfg.zb_endpoint}});
}
