{
    ezb_zha_dimmable_light_config_t {{prefix_lc}}_cfg = EZB_ZHA_DIMMABLE_LIGHT_CONFIG();
    ...
    ezb_af_ep_desc_t ep_desc =
        ezb_zha_create_dimmable_light({{cfg.zb_endpoint}}, &{{prefix_lc}}_cfg);
    if (ep_desc == EZB_INVALID_AF_EP_DESC) {
        ESP_LOGE(TAG, "{{prefix_lc}}: failed to create Zigbee Level Controllable Output endpoint");
        return ESP_ERR_NO_MEM;
    }
    ezb_err_t err =
        ezb_af_ep_desc_set_app_device_id(ep_desc, EZB_ZHA_LEVEL_CONTROLLABLE_OUTPUT_DEVICE_ID);
    if (err == EZB_ERR_NONE) {
    ...
    ESP_LOGI(TAG, "{{prefix_lc}}: Zigbee Level Controllable Output endpoint %d", {{cfg.zb_endpoint}});
}
