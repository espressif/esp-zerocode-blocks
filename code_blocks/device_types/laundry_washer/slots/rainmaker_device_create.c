{
    s_{{prefix_lc}}_device = esp_rmaker_device_create(
        "{{cfg.label}}", ESP_RMAKER_DEVICE_WASHER, NULL);
    ...
    if (s_{{prefix_lc}}_mode_param) {
        esp_rmaker_param_add_ui_type(s_{{prefix_lc}}_mode_param, ESP_RMAKER_UI_SLIDER);
    ...
    esp_rmaker_device_assign_primary_param(s_{{prefix_lc}}_device, s_{{prefix_lc}}_mode_param);
    esp_rmaker_node_add_device(node, s_{{prefix_lc}}_device);
    ESP_LOGI(TAG, "{{prefix_lc}}: RainMaker Washer device '{{cfg.label}}'");
}
