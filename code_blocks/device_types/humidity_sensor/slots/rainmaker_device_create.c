{
    ...
    /* No standard humidity param type — custom read-only float in %RH. */
    s_{{prefix_lc}}_humidity_param = esp_rmaker_param_create(
        "Humidity", NULL, esp_rmaker_float(init.i16 / 100.0f), PROP_FLAG_READ);
    esp_rmaker_device_add_param(s_{{prefix_lc}}_device, s_{{prefix_lc}}_humidity_param);
    esp_rmaker_device_assign_primary_param(s_{{prefix_lc}}_device, s_{{prefix_lc}}_humidity_param);
    esp_rmaker_node_add_device(node, s_{{prefix_lc}}_device);
    ESP_LOGI(TAG, "{{prefix_lc}}: RainMaker Humidity Sensor device '{{cfg.label}}'");
}
