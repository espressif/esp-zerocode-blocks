{
    /* No standard energy-management device type exists — use esp.device.other.
     * The power param is read-only telemetry, so no write callback is registered. */
    ...
    /* No standard param type for power measurement — custom (NULL type) param. */
    ...
    ESP_LOGI(TAG, "{{prefix_lc}}: RainMaker Energy Management device '{{cfg.label}}'");
}
