{
    ...
    uint32_t {{prefix_lc}}_init_w = init.u32 / 1000;
    int16_t {{prefix_lc}}_power_w = {{prefix_lc}}_init_w > 32767 ? 32767 : (int16_t){{prefix_lc}}_init_w;
    ...
    ESP_LOGI(TAG, "{{prefix_lc}}: Zigbee Electrical Measurement (energy mgmt) endpoint %d", {{cfg.zb_endpoint}});
}
