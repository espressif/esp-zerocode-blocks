...
if (param_id == {{cfg.brightness_param}}) {
    ...
    esp_matter::attribute::update(s_{{prefix_lc}}_endpoint_id,
        ...
        chip::app::Clusters::LevelControl::Attributes::CurrentLevel::Id, &mval);
    s_from_driver = false;
}
if (param_id == {{cfg.color_temp_param}}) {
    s_from_driver = true;
    esp_matter_attr_val_t mval = esp_matter_uint16(val.u16);
    esp_matter::attribute::update(s_{{prefix_lc}}_endpoint_id,
        chip::app::Clusters::ColorControl::Id,
        chip::app::Clusters::ColorControl::Attributes::ColorTemperatureMireds::Id, &mval);
    s_from_driver = false;
}
