if (endpoint == s_{{prefix_lc}}_zb_endpoint && message->in.attribute.data.value) {
    if (cluster_id == EZB_ZCL_CLUSTER_ID_ON_OFF &&
        ...
        app_driver_set_param({{cfg.brightness_param}}, pv, s_handle);
               ...
               attr_id == EZB_ZCL_ATTR_COLOR_CONTROL_COLOR_TEMPERATURE_MIREDS_ID &&
               message->in.attribute.data.type == EZB_ZCL_ATTR_TYPE_UINT16) {
        /* ZCL ColorTemperatureMireds and the driver param are both mireds u16. */
        app_driver_param_val_t pv = { .u16 = *(uint16_t *)message->in.attribute.data.value };
