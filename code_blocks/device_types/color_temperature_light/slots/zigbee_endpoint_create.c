{
    ...
    {{prefix_lc}}_cfg.color_cfg.color_mode =
        EZB_ZCL_COLOR_CONTROL_COLOR_MODE_COLOR_TEMPERATURE_MIREDS;
    {{prefix_lc}}_cfg.color_cfg.enhanced_color_mode =
        EZB_ZCL_COLOR_CONTROL_ENHANCED_COLOR_MODE_COLOR_TEMPERATURE_MIREDS;
    ...
    if (ep_desc == EZB_INVALID_AF_EP_DESC) {
        ESP_LOGE(TAG, "{{prefix_lc}}: failed to create Zigbee CCT Light endpoint");
    ...
    uint16_t {{prefix_lc}}_ct = init.u16 ? init.u16
        : EZB_ZCL_COLOR_CONTROL_COLOR_TEMPERATURE_MIREDS_DEFAULT_VALUE;
    ezb_err_t err = ezb_zcl_color_control_cluster_desc_add_attr(
        color_desc, EZB_ZCL_ATTR_COLOR_CONTROL_COLOR_TEMPERATURE_MIREDS_ID, &{{prefix_lc}}_ct);
    uint16_t {{prefix_lc}}_ct_min =
        EZB_ZCL_COLOR_CONTROL_COLOR_TEMP_PHYSICAL_MIN_MIREDS_DEFAULT_VALUE;
    if (err == EZB_ERR_NONE) {
        err = ezb_zcl_color_control_cluster_desc_add_attr(
            color_desc, EZB_ZCL_ATTR_COLOR_CONTROL_COLOR_TEMP_PHYSICAL_MIN_MIREDS_ID,
            &{{prefix_lc}}_ct_min);
    }
    uint16_t {{prefix_lc}}_ct_max =
        EZB_ZCL_COLOR_CONTROL_COLOR_TEMP_PHYSICAL_MAX_MIREDS_DEFAULT_VALUE;
    if (err == EZB_ERR_NONE) {
        err = ezb_zcl_color_control_cluster_desc_add_attr(
            color_desc, EZB_ZCL_ATTR_COLOR_CONTROL_COLOR_TEMP_PHYSICAL_MAX_MIREDS_ID,
            &{{prefix_lc}}_ct_max);
    }
    if (err == EZB_ERR_NONE) {
    ...
    ESP_LOGI(TAG, "{{prefix_lc}}: Zigbee CCT Light endpoint %d", {{cfg.zb_endpoint}});
}
