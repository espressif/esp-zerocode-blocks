{
    ...
    snprintf(s_{{prefix_lc}}_ct_state_topic, sizeof(s_{{prefix_lc}}_ct_state_topic),
             "zerocode/%s/{{prefix_lc}}/ct/state", s_node_id);
    snprintf(s_{{prefix_lc}}_ct_cmd_topic, sizeof(s_{{prefix_lc}}_ct_cmd_topic),
             "zerocode/%s/{{prefix_lc}}/ct/cmd", s_node_id);
    {
        char disc_topic[96];
        ...
        snprintf(cfg_json, sizeof(cfg_json),
                 "{\"name\":\"{{cfg.label}}\",\"unique_id\":\"zc_%s_{{prefix_lc}}\",\"availability_topic\":\"%s\",\"device\":{\"identifiers\":[\"zc_%s\"],\"name\":\"ZeroCode %s\",\"manufacturer\":\"ZeroCode AI\"},\"state_topic\":\"%s\",\"command_topic\":\"%s\",\"brightness_state_topic\":\"%s\",\"brightness_command_topic\":\"%s\",\"brightness_scale\":254,\"color_temp_state_topic\":\"%s\",\"color_temp_command_topic\":\"%s\"}",
                 s_node_id,
                 s_status_topic,
                 s_node_id,
                 s_node_id,
                 ...
                 s_{{prefix_lc}}_ct_state_topic,
                 s_{{prefix_lc}}_ct_cmd_topic);
    ...
    esp_mqtt_client_subscribe(s_client, s_{{prefix_lc}}_ct_cmd_topic, 1);
    {
