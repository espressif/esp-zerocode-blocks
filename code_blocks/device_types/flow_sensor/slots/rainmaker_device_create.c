{
    ...
    /* No standard flow param type — custom read-only float in m³/h. */
    s_{{prefix_lc}}_flow_param = esp_rmaker_param_create(
        "Flow", NULL, esp_rmaker_float(init.u16 / 10.0f), PROP_FLAG_READ);
    esp_rmaker_device_add_param(s_{{prefix_lc}}_device, s_{{prefix_lc}}_flow_param);
    esp_rmaker_device_assign_primary_param(s_{{prefix_lc}}_device, s_{{prefix_lc}}_flow_param);
    esp_rmaker_node_add_device(node, s_{{prefix_lc}}_device);
    ESP_LOGI(TAG, "{{prefix_lc}}: RainMaker Flow Sensor device '{{cfg.label}}'");
}
