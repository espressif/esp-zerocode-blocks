{
    ...
    app_driver_get_param({{cfg.run_mode_param}}, &init);
    s_{{prefix_lc}}_run_mode_param = esp_rmaker_param_create(
        "Run Mode", ESP_RMAKER_PARAM_MODE,
        esp_rmaker_int(init.u8), PROP_FLAG_READ | PROP_FLAG_WRITE);
    if (s_{{prefix_lc}}_run_mode_param) {
        esp_rmaker_param_add_ui_type(s_{{prefix_lc}}_run_mode_param, ESP_RMAKER_UI_SLIDER);
        esp_rmaker_param_add_bounds(s_{{prefix_lc}}_run_mode_param,
            esp_rmaker_int(0), esp_rmaker_int(2), esp_rmaker_int(1));
    }
    esp_rmaker_device_add_param(s_{{prefix_lc}}_device, s_{{prefix_lc}}_run_mode_param);
    ...
    esp_rmaker_device_assign_primary_param(s_{{prefix_lc}}_device, s_{{prefix_lc}}_run_mode_param);
    esp_rmaker_node_add_device(node, s_{{prefix_lc}}_device);
    ESP_LOGI(TAG, "{{prefix_lc}}: RainMaker Robot Vacuum device '{{cfg.label}}'");
}
