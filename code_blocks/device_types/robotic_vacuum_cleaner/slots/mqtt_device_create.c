{
             ...
             "zerocode/%s/{{prefix_lc}}/set", s_node_id);
    {
        ...
        snprintf(cfg_json, sizeof(cfg_json),
                 "{\"name\":\"{{cfg.label}}\",\"unique_id\":\"zc_%s_{{prefix_lc}}\",\"availability_topic\":\"%s\",\"device\":{\"identifiers\":[\"zc_%s\"],\"name\":\"ZeroCode %s\",\"manufacturer\":\"ZeroCode AI\"},\"state_topic\":\"%s\",\"command_topic\":\"%s\",\"options\":[\"idle\",\"cleaning\",\"mapping\"]}",
                 s_node_id,
                 s_status_topic,
                 s_node_id,
                 s_node_id,
