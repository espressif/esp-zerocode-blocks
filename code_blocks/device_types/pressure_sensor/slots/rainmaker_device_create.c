{
    ...
    /* No standard pressure param type — custom read-only float in kPa. */
    s_{{prefix_lc}}_pressure_param = esp_rmaker_param_create(
        "Pressure", NULL, esp_rmaker_float(init.i16 / 10.0f), PROP_FLAG_READ);
    esp_rmaker_device_add_param(s_{{prefix_lc}}_device, s_{{prefix_lc}}_pressure_param);
    esp_rmaker_device_assign_primary_param(s_{{prefix_lc}}_device, s_{{prefix_lc}}_pressure_param);
    esp_rmaker_node_add_device(node, s_{{prefix_lc}}_device);
    ESP_LOGI(TAG, "{{prefix_lc}}: RainMaker Pressure Sensor device '{{cfg.label}}'");
}
