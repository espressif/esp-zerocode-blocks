if (endpoint == s_{{prefix_lc}}_zb_endpoint && message->in.attribute.data.value) {
    ...
    } else if (cluster_id == EZB_ZCL_CLUSTER_ID_COLOR_CONTROL &&
               attr_id == EZB_ZCL_ATTR_COLOR_CONTROL_CURRENT_HUE_ID &&
               message->in.attribute.data.type == EZB_ZCL_ATTR_TYPE_UINT8) {
        /* ZCL CurrentHue and the driver hue are both 0-254. */
        app_driver_param_val_t pv = { .u8 = *(uint8_t *)message->in.attribute.data.value };
        app_driver_set_param({{cfg.hue_param}}, pv, s_handle);
    } else if (cluster_id == EZB_ZCL_CLUSTER_ID_COLOR_CONTROL &&
               attr_id == EZB_ZCL_ATTR_COLOR_CONTROL_CURRENT_SATURATION_ID &&
               message->in.attribute.data.type == EZB_ZCL_ATTR_TYPE_UINT8) {
        /* ZCL CurrentSaturation and the driver saturation are both 0-254. */
        app_driver_param_val_t pv = { .u8 = *(uint8_t *)message->in.attribute.data.value };
