{
    /* ADOPT, THEN CREATE. i2c_new_master_bus() fails on a port that already
       carries a bus, and on a `board:` product this slot is dropped entirely
       (bmgr.replaces_slots) — but a display/touch block configuring the same
       port from its own task is neither of those cases. Looking the port up
       first makes "two blocks want this bus" benign whichever one runs first,
       and it is what removed mosaico-touch-hub's double-configuration of I2C0.
    ...
    if (i2c_master_get_bus_handle((i2c_port_num_t){{prefix}}_I2C_PORT, &{{prefix_lc}}_i2c_bus) == ESP_OK) {
        ESP_LOGI(TAG, "I2C bus {{prefix_lc}}: port %d already configured — adopted", {{prefix}}_I2C_PORT);
    } else {
        ...
        if ({{prefix_lc}}_i2c_err != ESP_OK) {
            ESP_LOGE(TAG, "I2C bus {{prefix_lc}}: i2c_new_master_bus failed: %s", esp_err_to_name({{prefix_lc}}_i2c_err));
        } else {
            ...
            ESP_LOGI(TAG, "I2C bus {{prefix_lc}}: port %d SDA=%d SCL=%d (devices clock themselves; product default %uHz)",
