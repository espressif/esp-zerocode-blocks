{
    ...
    if ({{prefix_lc}}_i2c_attach() != ESP_OK) {
        ESP_LOGE(TAG, "SHT4x {{prefix_lc}}: not started — no I2C bus");
    } else {
        const esp_timer_create_args_t {{prefix_lc}}_args = {
            .callback = &{{prefix_lc}}_sht4x_poll_cb,
            .arg = NULL,
            .dispatch_method = ESP_TIMER_TASK,
            .name = "{{prefix_lc}}_sht4x",
            .skip_unhandled_events = true,
        };
        esp_timer_handle_t {{prefix_lc}}_timer = NULL;
        ESP_ERROR_CHECK(esp_timer_create(&{{prefix_lc}}_args, &{{prefix_lc}}_timer));
        ESP_ERROR_CHECK(esp_timer_start_periodic({{prefix_lc}}_timer, (uint64_t){{prefix}}_SHT4X_POLL_MS * 1000ULL));
        {{prefix_lc}}_sht4x_poll_cb(NULL);
        ESP_LOGI(TAG, "SHT4x {{prefix_lc}}: polling on I2C port %d addr 0x%02x", {{prefix}}_SHT4X_I2C_PORT, {{prefix}}_SHT4X_ADDR);
    }
}
