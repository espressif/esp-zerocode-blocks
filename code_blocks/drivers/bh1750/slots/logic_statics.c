...
static esp_err_t {{prefix_lc}}_i2c_attach(void)
{
    ...
    esp_err_t {{prefix_lc}}_err = i2c_master_get_bus_handle((i2c_port_num_t){{prefix}}_BH_PORT, &{{prefix_lc}}_bus);
    if ({{prefix_lc}}_err != ESP_OK) {
        ...
        ESP_LOGE(TAG, "{{prefix_lc}}: no I2C bus on port %d (%s) — needs a bus provider EARLIER in the product, or a board that owns the port",
                 (int){{prefix}}_BH_PORT, esp_err_to_name({{prefix_lc}}_err));
    ...
    {{prefix_lc}}_dev_cfg.device_address = {{prefix}}_BH_ADDR;
    {{prefix_lc}}_dev_cfg.scl_speed_hz = {{prefix}}_BH_FREQ_HZ;
    ...
    if ({{prefix_lc}}_err != ESP_OK) {
        ESP_LOGE(TAG, "{{prefix_lc}}: i2c_master_bus_add_device(0x%02x): %s",
                 (unsigned){{prefix}}_BH_ADDR, esp_err_to_name({{prefix_lc}}_err));
        s_{{prefix_lc}}_i2c_dev = NULL;
    }
    return {{prefix_lc}}_err;
}

static void {{prefix_lc}}_bh_poll_cb(void *arg)
{
    uint8_t buf[2] = {0};
    if (i2c_master_receive(s_{{prefix_lc}}_i2c_dev, buf, sizeof(buf), 50) != ESP_OK) return;
    uint16_t raw = ((uint16_t)buf[0] << 8) | buf[1];
    /* lux = raw / 1.2 = raw * 5 / 6 */
    uint32_t lux = ((uint32_t)raw * 5U) / 6U;
    if (lux > 0xFFFF) lux = 0xFFFF;
    app_driver_param_val_t v = { .u16 = (uint16_t)lux };
