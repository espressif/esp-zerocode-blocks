...
{
    if (!s_{{prefix_lc}}_strip) return;
    uint8_t r = 0, g = 0, b = 0;
    if (s_{{prefix_lc}}_on) {
        {{prefix_lc}}_hsb_to_rgb(s_{{prefix_lc}}_hue, s_{{prefix_lc}}_sat, s_{{prefix_lc}}_brightness, &r, &g, &b);
    }
    for (int i = 0; i < {{prefix}}_NUM_LEDS; ++i) {
        led_strip_set_pixel(s_{{prefix_lc}}_strip, i, r, g, b);
    }
    esp_err_t err = led_strip_refresh(s_{{prefix_lc}}_strip);
    if (err != ESP_OK) ESP_LOGE(TAG, "{{prefix_lc}}: led_strip_refresh: %s", esp_err_to_name(err));
}
