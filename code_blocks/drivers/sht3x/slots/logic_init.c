{
    ...
    if ({{prefix_lc}}_i2c_attach() != ESP_OK) {
        ESP_LOGE(TAG, "SHT3x {{prefix_lc}}: not started — no I2C bus");
    } else {
        const esp_timer_create_args_t {{prefix_lc}}_args = {
            .callback = &{{prefix_lc}}_sht3_poll_cb,
            .arg = NULL,
            .dispatch_method = ESP_TIMER_TASK,
            .name = "{{prefix_lc}}_sht3",
            .skip_unhandled_events = true,
        };
        esp_timer_handle_t {{prefix_lc}}_timer = NULL;
        ESP_ERROR_CHECK(esp_timer_create(&{{prefix_lc}}_args, &{{prefix_lc}}_timer));
        ESP_ERROR_CHECK(esp_timer_start_periodic({{prefix_lc}}_timer, (uint64_t){{prefix}}_SHT3_POLL_MS * 1000ULL));
        {{prefix_lc}}_sht3_poll_cb(NULL);
        ESP_LOGI(TAG, "SHT3x {{prefix_lc}}: I2C port %d addr 0x%02x", {{prefix}}_SHT3_PORT, {{prefix}}_SHT3_ADDR);
    }
}
