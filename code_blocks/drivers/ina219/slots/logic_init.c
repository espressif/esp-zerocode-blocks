{
    ...
    if ({{prefix_lc}}_i2c_attach() != ESP_OK) {
        ESP_LOGE(TAG, "INA219 {{prefix_lc}}: not started — no I2C bus");
    } else {
        const esp_timer_create_args_t {{prefix_lc}}_args = {
            .callback = &{{prefix_lc}}_ina_poll_cb,
            .arg = NULL,
        ...
        ESP_ERROR_CHECK(esp_timer_start_periodic({{prefix_lc}}_timer, (uint64_t){{prefix}}_INA_POLL_MS * 1000ULL));
        {{prefix_lc}}_ina_poll_cb(NULL);
        ESP_LOGI(TAG, "INA219 {{prefix_lc}}: polling on I2C port %d addr 0x%02x", {{prefix}}_INA_PORT, {{prefix}}_INA_ADDR);
    }
}
