{
    ...
    /* Field-by-field instead of ESP_LCD_TOUCH_IO_I2C_CST9217_CONFIG(): the
     * macro's designated-initializer ORDER is C-legal but rejected by C++
     * (this file compiles as C++). Values mirror the macro; the CST92xx
     * family uses 16-bit register addressing (lcd_cmd_bits = 16). */
    esp_lcd_panel_io_handle_t {{prefix_lc}}_tio = NULL;
    esp_lcd_panel_io_i2c_config_t {{prefix_lc}}_tio_cfg = {};
    {{prefix_lc}}_tio_cfg.dev_addr = ESP_LCD_TOUCH_IO_I2C_CST9217_ADDRESS;
    {{prefix_lc}}_tio_cfg.scl_speed_hz = 400000;
    {{prefix_lc}}_tio_cfg.control_phase_bytes = 1;
    {{prefix_lc}}_tio_cfg.dc_bit_offset = 0;
    {{prefix_lc}}_tio_cfg.lcd_cmd_bits = 16;
    ...
    ESP_ERROR_CHECK(esp_lcd_touch_new_i2c_cst9217({{prefix_lc}}_tio, &{{prefix_lc}}_tp_cfg, &{{prefix_lc}}_tp));
    ...
    if ({{prefix_lc}}_indev == NULL) {
        ...
        ESP_LOGI(TAG, "Touch {{prefix_lc}}: CST92xx on I2C%d", {{prefix}}_TOUCH_I2C_PORT);
    }
}
