...
/* Vendored from esp-tflite-micro examples/hello_world/main/model.cc (the TFLM
 * "hello world" sine regressor, 2488 bytes, a single FullyConnected graph).
 * Renamed to zc_tflite_reference_model and declared in app_tflite.h so this
 * framework links and runs an inference with no product-supplied model.
 *
 * REFERENCE / TEST ONLY. It regresses sin(x); on the zeroed input
 * drivers/ml_classifier feeds by default the result means nothing. It is here
 * for CI and ml-inference-demo, not for a shipping product.
 *
 * Not a cost to carry: point a block's model_symbol at a real model and this
 * array goes unreferenced, and --gc-sections drops it. Do NOT delete it to
 * "save space" — without it the framework no longer links standalone and CI
 * has nothing to compile. frameworks/ml/block.yml documents how a real model
 * gets in. */

#include "app_tflite.h"

// Keep model aligned to 8 bytes to guarantee aligned 64-bit accesses.
alignas(8) const unsigned char zc_tflite_reference_model[] = {
    ...
    0x00, 0x00, 0x00, 0x09};
const int zc_tflite_reference_model_len = 2488;
